...
import "../businessgallery/BusinessGallery.css"
...
export const BusinessGallery = () => {
  ...
  const getLocalItem = () => {
    let data = (JSON.parse(localStorage.getItem("businessgallery"))).filter(d=> d.isDeleted == false);
  ...
  const handleDelete = (e) => {
    console.log(e);
    const gallery = gallerydata.find(a => a.id !== e.id);
    ...
    console.log(gallerydata)
    localStorage.setItem("businessgallery", JSON.stringify(gallerydata))
  }
  ...
  return (
    <div className="page-information-container">
      <div className="page-header"><label>
        ...
        <div className='page-breadscrumb'>
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Link
              underline="hover" area-current="page" style={{ color: "black" }} onClick={() => { navigate.push("/admindashboard/businessgallery") }}>
              BusinessGallery
            </Link>
      ...
      <div className="businessgallery-container">
        <div className="businessgallery-container-button">
          <NavLink to="businessgallery/createbusinessgallery">
            <Button variant="contained" color="success" size='small' className='btn-create'>
              <Typography variant='caption' className='btn-success-font'>Create New business gallery</Typography>
            </Button>
          </NavLink>
         
        </div>
        <div className="businessgallery-table-container">
          <TableContainer sx={{ boxShadow: "box-shadow:  3px 1px -2px rgba(0,0,0,0.2), 0px 2px 2px 0px rgba(0,0,0,0.14), 0px 1px 5px 0px rgba(0,0,0,0.12)" }}>
            <Table stickyHeader aria-label='sticky table' sx={{ maxHeight: 440 }} size='small'>
              ...
              {gallerydata.length > 0 &&
                <TableBody>
                  {gallerydata.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                    .map((e) => {
                      ...
                      if (e.isDeleted == false) return (
                        <StyledTableRow hover role="checkbox" tabIndex={-1} key={e.id}>
                          ...
                          <StyledTableCell>{e.galleryTitle}</StyledTableCell>
                          <StyledTableCell>{e.galleryDescription}</StyledTableCell>
                          <StyledTableCell><img src={e.galleryImage} width="100px" height="100px" /></StyledTableCell>
                          
                          <StyledTableCell>{convert(e.galleryStartDate)}</StyledTableCell>
                          <StyledTableCell>{convert(e.galleryEndDate)}</StyledTableCell>
                          <StyledTableCell>{e.isApproved ? "Approved" : "Not Approved"}</StyledTableCell>
                          <StyledTableCell>
                            <LinkRoute to={{
                              pathname: "/admindashboard/businessgallery/updatebusinessgallery/:id",
                              state: { idParam: e.id }
